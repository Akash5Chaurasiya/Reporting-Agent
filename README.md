...
### Configuring Ports and Hosts

By default, the frontend runs on port 3000 and the backend agent runs on port 8000. You can customize these via environment variables.

#### Backend (Python/FastAPI) Configuration

| Variable | Default | Description |
|----------|---------|-------------|
| `PORT` | `8000` | Port for the Python agent server |
| `CORS_ORIGINS` | `*` | Allowed CORS origins (comma-separated) |

The backend binds to `0.0.0.0` (all network interfaces), making it accessible from other machines on your network.

#### Frontend → Backend Connection

| Variable | Default | Description |
|----------|---------|-------------|
| `AGENT_URL` | `http://localhost:8000/` | Server-side URL for CopilotRuntime |
| `NEXT_PUBLIC_AGENT_URL` | `http://localhost:8000` | Client-side URL for direct API calls |
...
**Connect frontend to remote server:**
```bash
# Connect to a server at 192.168.1.100
AGENT_URL=http://192.168.1.100:8000/
NEXT_PUBLIC_AGENT_URL=http://192.168.1.100:8000
```

**Docker/Container network:**
```bash
# If backend is running in Docker with container name "agent"
...
```bash
toolbox --tools-file "tools.yaml" --log-level info --ui
```# Reporting-Agent
